const { Orders , Payment , OrderDetail , CardProduct , CardCode , BankAccount , sequelize } = require("../models");
      ...
      include: [
        {
          ...
          include: [
            {
              model: BankAccount,
              attributes: ["accountName","bankName","accountNumber","isDeleted"]
            }
          ]
        },
        {
...
exports.getOrderByOrderId = async (req, res, next) => {
  try {
    ...
    if(ordersById.length == 0) return res.status(400).json({message: "no orders by this orderId found"});
...
exports.getOrderByUserId = async (req, res, next) => {
  try {
    ...
    if(allOrdersByUserId.length == 0) return res.status(400).json({message: "no orders by this userId found"});
...
exports.placeOrder = async (req, res, next) => {
  ...
  try {
    ...
    const orderLists = await Promise.all(
      orderItems.map( async (item, index) => {
        ...
        if( +item.amount !== changeAmountToCardCode.length ) {
          throw new AppError(400, "can't place order to this product; not enough product amount to place this order");
        };
        ...
        if(!findCardProduct) {
          throw new AppError(400, "can't place order to this product; isDeleted:'DELETE' ");
        };
    ...
    for (let item of orderLists) {
      for ( i = 0 ; i < item.amount.length ; i++ ) {
        console.log(item)
        insertToOrderDetail.push({
          ...
          cardCodeId: item.amount[i].id,
          cardCodeNumber: item.amount[i].codeNumber,
          orderId: newOrder.id
        });
      }
    };
    ...
    const newOrderItems = await OrderDetail.bulkCreate(insertToOrderDetail, { transaction:transaction }); 
   ...
   // ทำให้ codeStatus: AVAILABLE > PENDING เลยทันที ; fromOrder ส่งไปเพื่อทำให้ <cardCode-editCardCodeStatus> ไม่ส่ง res ซ้ำ
    ...
    for (let item of orderLists) {
      for ( i = 0 ; i < item.amount.length ; i++ ) {
        findOrderedCardCodeIds.push(item.amount[i].id,);
      }
    };
