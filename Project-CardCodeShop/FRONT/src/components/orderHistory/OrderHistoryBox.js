...
{orderHistory?.map((item, index) => {
  ...
  return (
    ...
    {item.OrderDetails.map((item1, index) => {
       ...
       return (
          <table className="content-center-profile-historyBox-orderDetail-table" key={item1.id}>
            <tbody>
              <tr>
                <td>
                  <div className="content-center-profile-historyBox-orderDetail-table-left">
                    <img src={item1.CardCode.CardProduct.img} alt="item1" className="content-center-profile-historyBox-orderDetail-table-itemImg" />
                  </div>
                </td>
    ...
    {item.OrderDetails.reduce((acc2, item2, index) => {
      ...
      <div className="content-center-profile-historyBox-orderDetail-pre-footer">
        <div><h3>ราคารวม</h3></div>
        <div><h3>{
          (acc2.props) ? acc2 = acc2.props.children[1].props.children.props.children[0] + +item2.CardCode.CardProduct.price : +item2.CardCode.CardProduct.price
          } บาท</h3></div>
      </div>
      )
      }, 0)
  ...
  <div className="content-center-profile-historyBox-orderDetail-footer">
    <div><h3>Order Status: {item.paymentStatus}</h3></div>
