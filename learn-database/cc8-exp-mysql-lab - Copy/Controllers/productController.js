...
exports.createProduct = ( async (req, res, next) => {
    try {
        ...
        if ( !(+price > 0) ) return res.status(400).json({ message: 'price must be numeric and greater than zero' });
        if ( quantity === undefined ) return res.status(400).json({message: 'quantity is required'});
        if ( !(+quantity >= 0) ) return res.status(400).json({ message: 'quantity must be numeric and equal or greater than zero' });
...
exports.updateProduct = ( async (req, res, next) => {
    try {
        ...
        const product = await Product.updateById(id, { name, 
            price, 
            quantity, 
            supplier_id, 
            desc: desc ? desc: null
