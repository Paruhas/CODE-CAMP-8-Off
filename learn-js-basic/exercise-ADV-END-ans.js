...
TEST4 = sales.reduce((acc, item) => {
  ...
  if (acc[item.product.name]) {
    acc[item.product.name] = {
      ...
      total: acc[item.product.name].total + total
    }
  } else {
    acc[item.product.name] = {
      count: 1,
...
let saleProduct = sales.reduce(function (acc, value) {
  if (!acc[value.product.name]) {
    acc[value.product.name] = [value.product.model];
  } else if (!acc[value.product.model] || acc[value.product.model] ) {
